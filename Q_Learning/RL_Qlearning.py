...
class GridWorld:
    ...
    def generate_obstacles(self):
        ...
        for _ in range(self.num_obstacles):
            while True:
                obstacle = (np.random.randint(self.size),
        ...
        while episode < self.episodes:
            ...
            while not done:
                ...
                if done and state == self.env.goal:  # Check if the agent has reached the goal
                    # Store the start of the new episode
                    starts.append(len(states))
                    rewards.append(total_reward)
                    # Store the number of steps for this episode
...
for i in range(10):
    ...
    # Get starts and steps_per_episode as well
    ...
    def update(i):
        ...
        current_episode = next(
            (j for j, start in enumerate(starts) if start > i), len(starts)) - 1
        ...
        ax.set_title(f"Iteration: {
                     current_episode+1}, Total Reward: {cumulative_reward:.2f}, Steps: {steps}")
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=range(len(states)), repeat=False)
